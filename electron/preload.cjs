...
const windowControls = Object.freeze({
  ...
  setFullscreen: (enabled) => {
    if (typeof enabled !== 'boolean')
      return Promise.reject(new TypeError('Fullscreen state must be a boolean'));
  ...
  setWindowSize: (width, height) => {
    ...
    if (!supported) return Promise.reject(new RangeError('Unsupported desktop window size'));
    return ipcRenderer.invoke(IPC.setWindowSize, width, height);
  },
  quit: () => ipcRenderer.invoke(IPC.quit),
  onFullscreenChanged: (listener) => {
    if (typeof listener !== 'function') throw new TypeError('Fullscreen listener must be a function');
